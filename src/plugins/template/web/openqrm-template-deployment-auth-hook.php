	...
	*/
	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		if ($vm_host_resource->id != $storage_resource->id) {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-lxc-lvm-deployment-auth-hook.php", "Appliance $appliance_id image IS NOT available on this template host, $storage_resource->id not equal $vm_host_resource->id !! Assuming SAN Backend", "", "", 0, 0, $appliance_id);
		} else {
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-lxc-lvm-deployment-auth-hook.php", "Appliance $appliance_id image IS available on this template host, $storage_resource->id equal $vm_host_resource->id", "", "", 0, 0, $appliance_id);
		}
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-lxc-lvm-deployment-auth-hook.php", "Authenticating ".$image_name." / ".$image_rootdevice." on ".$vm_host_resource->ip." to resource ".$resource_ip.".", "", "", 0, 0, $appliance_id);
				$auth_start_cmd = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/template/bin/openqrm-template auth -n ".$image_name." -r ".$image_rootdevice." -i ".$resource_ip." -u ".$openqrm_admin_user->name." -p ".$openqrm_admin_user->password." --openqrm-cmd-mode background";
