...
function remove_kvm_lvm_deployment($cloud_image_id) {
	...
	if ($cloudimage->resource_id > 0) {
		...
		// san backend ?
		if ($vm_host_resource->id != $resource->id) {
			$event->log("remove_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Image ".$image_id." IS NOT available on this kvm host, ".$resource->id." not equal ".$vm_host_resource->id." !! Assuming SAN Backend", "", "", 0, 0, 0);
		} else {
			$event->log("remove_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Image ".$image_id." IS available on this kvm host, ".$resource->id." equal ".$vm_host_resource->id.".", "", "", 0, 0, 0);
		}
	}
	$image_remove_clone_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/kvm/bin/openqrm-kvm remove -n ".$image_location_name." -v ".$volume_group." -t ".$deployment->type." -u ".$openqrm_admin_user->name." -p ".$openqrm_admin_user->password." --openqrm-cmd-mode background";
...
function resize_kvm_lvm_deployment($cloud_image_id, $resize_value) {
	...
	if ($cloudimage->resource_id > 0) {
		...
		// san backend ?
		if ($vm_host_resource->id != $resource->id) {
			$event->log("resize_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Image ".$image_id." IS NOT available on this kvm host, ".$resource->id." not equal ".$vm_host_resource->id." !! Assuming SAN Backend", "", "", 0, 0, 0);
		} else {
			$event->log("resize_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Image ".$image_id." IS available on this kvm host, ".$resource->id." equal ".$vm_host_resource->id, "", "", 0, 0, 0);
		}
	}

	$image_resize_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/kvm/bin/openqrm-kvm resize -n ".$image_location_name." -v ".$volume_group." -m ".$resize_value." -t ".$deployment->type." -u ".$openqrm_admin_user->name." -p ".$openqrm_admin_user->password." --openqrm-cmd-mode background";
