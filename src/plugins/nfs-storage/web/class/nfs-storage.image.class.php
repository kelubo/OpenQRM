			...
			switch ($image_command) {
				case 'add':
					...
					if (strlen($image->id)) {
						...
						$ident_file = $this->openqrm->get('basedir').'/plugins/nfs-storage/web/storage/'.$this->resource->id.'.nfs.'.$image_name.'.adapt';
						$get_ident_command  = $this->openqrm->get('basedir').'/plugins/nfs-storage/bin/openqrm-nfs-storage adapt';
