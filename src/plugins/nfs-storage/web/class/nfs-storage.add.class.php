...
var $lang = array();
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command     = $this->openqrm->get('basedir')."/plugins/nfs-storage/bin/openqrm-nfs-storage add -n $name";
	...
	//--------------------------------------------
	function set_max() {
		$statfile = $this->openqrm->get('basedir').'/plugins/nfs-storage/web/storage/'.$this->resource->id.'.nfs.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
