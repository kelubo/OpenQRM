...
*/
function get_sshterm_dashboard_quicklink($html) {
	// create <i> tag for button icon	
	$quicklink_icon = $html->i();
	$quicklink_icon->css = 'glyphicons-icon keyboard-wireless';
...
*/	
	...
	$quicklink_label->add('SSH Manager');
...
//	$quicklink->label = $quicklink_icon->get_string() . $quicklink_label->get_string() . $quicklink_corner->get_string() . $quicklink_corner_label->get_string();
	...
	$quicklink->css = 'btn quicklink sshterm-quicklink';
	$quicklink->href = 'index.php?plugin=sshterm&controller=sshterm';
