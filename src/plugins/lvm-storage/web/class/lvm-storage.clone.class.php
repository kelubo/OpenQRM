...
var $lang = array();
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->openqrm->get('basedir').'/plugins/lvm-storage/bin/openqrm-lvm-storage clone';
			...
			if(isset($error)) {
				...
				$file = $this->openqrm->get('basedir').'/plugins/lvm-storage/web/storage/'.$resource->id.'.lvm.'.$name.'.sync_progress';
				if(file_exists($file)) {
					unlink($file);
				}
